#ifndef ACCEPTATRON_T_GETOPT_H
#define ACCEPTATRON_T_GETOPT_H
...
#include <stddef.h>
#include <string.h>
/* AT&T public domain implementation from the Internet;
   To make the implementation acceptable for a header, I've added "inline",
   changed from a K&R function definition to a standard C definition, commented
   out the definition for NULL, and added #includes.  The code is otherwise
   unchanged.  I am not convinced that my changes are significant for
   copyright purposes.
*/
...
inline int getopt(int argc, char **argv, *opts)
{
	static int sp = 1;
	register int c;
	register char *cp;
