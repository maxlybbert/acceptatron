...
F<getopt.h>.  If the this project's F<getopt.h> is C<#include>d in angle
...
fixed this by using C<#include "getopt.h">.  If that is too subtle, you
could rename the file to something like F<acceptatron_getopt.h>.
...
It seems like a safe bet that programmers interested in using TAP testing
frameworks already have some version of Perl and associated programs (e.g.,
...
The acceptatron is a single-header library and does not need a makefile.
Both F<Makefile.PL> and the generated F<Makefile> should work on Windows.
Unfortunately, I don't have access to a Windows computer.  If F<Makefile.PL>
doesn't run on Windows, or the generated makefile doesn't work, please
report it as a bug.
